# v1.0

## 1.3.2

新增“市场”页面，可按类型浏览最近入库的数据并继续加载更早记录。

档案查询增加通用 XML 结构摘要：读取真实的 `Ship` 与 `Runtime` 文档，展示根节点属性、节点统计和一级分块；格式化视图也支持对存档节点分块、折叠和着色。

## 1.3.1

添加已通过 XML/飞船结构校验的飞船工程分析接口

仪表板档案页新增部件、质量、燃料、推进、级序和连接信息，
并加入 XML 语法高亮与分区折叠查看

## 1.3.0

补上了一套完整的 XML 解析模块, 现在 `Ship` 和 `Save` 都可以解析成结构化 Rust 数据, 也支持再写回 XML

新的 `xml_part` 模块把 raw/model/parse/write/error 分开了
`Save` 里的 `ShipNode -> Ship` 也抽到了共享的船体结构上, 后面继续做分析和校验会顺手很多

同时给 `DbData` 和下载结果补上了直接解析 XML 的入口
后面不需要再到处手写 `quick_xml` 的临时解析逻辑了

顺手升级了一批依赖版本, 包括 `reqwest 0.13`、`quick-xml 0.39`、`tokio 1.51`、`clap 4.6` 和 `toml 1.1.2`
调整了对应的配置和兼容代码, 现在编译和 XML round-trip 测试都已经重新跑通

## 1.2.11

重新整理了 dashboard 的页面样式

修复了左下角深色模式按钮样式丢失的问题
补上了悬浮按钮的定位、交互状态和移动端显示

顺手补了深浅色切换按钮的可访问性文本
现在系统主题变化时按钮图标也会同步更新

## 1.2.8-9

添加了一些统计数据, api 调用计数/网页请求计数

## 1.2.7

开始写自己的迁移脚本

## 1.2.6

1.3 会迁移到自己写的脚本的
不过现在还是用 sea orm 的
数据库表里添加了 time 字段
用来记录时间
添加了新的检查数据库内数据有效性的 api
现在会检测了
去掉了 `get-save`
在网页 api 里添加了是否通过 xml 检测的选项
也在 dashboard 上添加了用时显示

## 1.2.5

额, 准备迁移到自己写的迁移脚本
然后遇到了 sea_orm 不支持 xml 类型的问题
先发一版再说

顺便修复了web界面的一些可能导致 panic 的 unwrap

## 1.2.4

又修复了一遍剩下的问题

## 1.2.3

修复了 1080p 屏幕下的一些排版问题(比如太宽啦啊啊)
还有手机上的显示问题

## 1.2.2

修复了不跳转根路径的问题
给网页加上了图标(还是DR的)

## 1.2.1

我甚至为 IE 优化了一下 (加了一个 `-ms-flex-direction`)

## 1.2.0

加入了不少 api
甚至加入了 `/dashboard` 页面
修改了之前的那堆 api 的路径和返回值

## 1.1.1

修复了 serve 模式如果在配置文件里禁用, 实际上还是会启动的问题

## 1.1.0

现在加了个 serve 模式, 可以用来提供一个 api
API 信息参考 readme

## 1.0.2

现在 ctrl-c 可以直接退出, 不用等了

## 1.0.1

现在支持 musl build 了 (openssl 拜拜了您内)
